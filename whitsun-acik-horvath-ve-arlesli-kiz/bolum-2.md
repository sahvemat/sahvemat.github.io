---
layout: post
title: "Whitsun Açık, Horvath ve Arlesli Kız 02"
author: "Tarık Selbes"
section: "Turnuva Günlüğü"
event: "Whitsun Açık · Münih Satranç Festivali"
date: 2026-06-03
permalink: /posts/whitsun-acik-horvath-ve-arlesli-kiz/bolum-2/
---

{% include chapter-toc.html dataset="whitsun_toc" %}

## <span class="chapter-num">02</span> İlk Turlar

Yön hareketi gibi benim de ne kadar başarılı olduğum sorgulanabilir kuşkusuz ama en azından ilk tur kazanarak başladım:

<div class="post-game" id="tur-1" data-pgn="{{ '/assets/pgn/munich/munich-r1.pgn' | relative_url }}"></div>

İkinci tur rakibim genç Bulgar GM Petkov'du ve yeni bir açılış denemek, bu açılışta deneyim kazanmak için bu tur iyi bir fırsattı. Elbette açılış tercihim eleştirilebilir ama oyunu kaybetmem açılıştan çok sonrasındaki hatalarım neticesinde oldu, dolayısıyla burada esas fatura oyun gücüme çıkmalı. Bazen pasif oynamak da gerekiyor!

<div class="post-game" id="tur-2" data-pgn="{{ '/assets/pgn/munich/munich-r2.pgn' | relative_url }}"></div>

![Turnuva salonu]({{ '/assets/img/munich/salon.jpg' | relative_url }} "Moosach Lisesi'ndeki turnuva salonundan bir görünüm."){: loading="lazy" decoding="async"}

Üçüncü tur eşleştiğim Letonyalı Saveljevs ilginç bir şekilde benden bir yaş büyüktü, genelde rakiplerimin yaşımın üçte biri civarında olmasına alıştığım için bu hoş sürprizi değerlendirmem şarttı. Her ne kadar bilgisayar bazı delikler bulsa da en azından hamle hamle ilerleterek hızlıca bakıldığında tek taraflı ve iyi bir oyun gibi göründüğünü söyleyebilirim:

<div class="post-game" id="tur-3" data-pgn="{{ '/assets/pgn/munich/munich-r3.pgn' | relative_url }}"></div>

<div class="next-section-nav-wrap">
<a class="next-section-nav" href="{{ '/posts/whitsun-acik-horvath-ve-arlesli-kiz/bolum-3/' | relative_url }}">
  <span class="next-section-label">Sıradaki Bölüm</span>
  <span class="next-section-title"><span class="next-section-text">Münih ve Allahsız Gençlik</span> <span class="next-section-arrow">→</span></span>
</a>
</div>
